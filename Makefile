...
TARGET = main
TARGET_EXT = elf
CMSIS_PATH = ../kinetis_klxx_gcc/cmsis
LD_SCRIPT = $(CMSIS_PATH)/$(FOLDER_NAME)/$(LINKER_NAME).ld
...
# directories
INC_DIRS = $(CMSIS_PATH) $(CMSIS_PATH)/$(FOLDER_NAME) .
# app headers directories (remove comment and add more files)
#INC_DIRS +=

SRC_DIRS = $(CMSIS_PATH) $(CMSIS_PATH)/$(FOLDER_NAME) .
...
COMPILER_OPTIONS  = -g -ggdb -Wall -fno-strict-aliasing #-Os 
...
# Linker options
LD_OPTIONS = -mcpu=$(CPU) -m$(INSTRUCTION_MODE) -T $(LD_SCRIPT) $(INC_DIRS_F) #-Os
